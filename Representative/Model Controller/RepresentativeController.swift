...
class RepresentativeController {
    ...
    static func searchRepresentativesForZip(zipcode: String, completion: @escaping ([Representative]) -> Void) {
        let baseURL = URL(string: "http://whoismyrepresentative.com/getall_mems.php")
        guard let theURL = baseURL else {return completion([])}
        let urlParameters = ["zip": "\(zipcode)", "output": "json"]
        let queryItems = urlParameters.compactMap( { URLQueryItem(name: $0.key, value: $0.value)})
        var urlComponents = URLComponents(url: theURL, resolvingAgainstBaseURL: true)
        urlComponents?.queryItems = queryItems
